#!/usr/bin/env python3
"""PROTOTYPE (throwaway) — generic Suite runner for the deck-test artifact-shape question.

Candidate C: the Suite is data (suite.yaml); this runner is a fixed skill asset that
interprets any Suite. Same file runs in Claude Code (files on disk) and in claude.ai
chat's code-execution sandbox (suite + deck pasted into the sandbox as text).

stdlib only, on purpose: the chat sandbox guarantees nothing else. The YAML subset
parser below exists so the Suite stays human-readable without buying a dependency.

Usage:
  python3 check_deck.py --deck decks/final.txt          # run, print report Block
  python3 check_deck.py --render-checklist              # candidate A, generated from the same Suite
  python3 check_deck.py --dump-json                     # parsed Suite as JSON (demo.html embed)
"""
...
def report(suite, deck_name, results, unknown, oracle_size):
...
# ---------- candidate A: checklist rendered from the same Suite ----------
...
def main():
    ...
    ap.add_argument("--suite", default=os.path.join(HERE, "suite.yaml"))
    ap.add_argument("--deck", default=os.path.join(HERE, "decks", "final.txt"))
    ap.add_argument("--oracle", default=os.path.join(HERE, "oracle.json"))
    ap.add_argument("--collection", default=os.path.join(HERE, "collection.csv"))
    ...
    print(report(suite, deck_name, results, unknown, len(oracle)))
