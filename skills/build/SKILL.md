...
## Tables: Seats build sequentially

A Table's Seats run through this exact loop, unchanged, one Seat at a time in the Table Brief's seat order — seat order = build order = contention priority. Each Seat's session reads only its own Brief; what makes it a table build is one extra input: every earlier Seat's finished Deck Block, passed as `--table-mate` (repeatable) to both `availability.py` and `ship_deck.py`. A table-mate's copies count as committed to that Deck — the arithmetic declines them in the same honest sentence ("wanted Sol Ring; all copies committed to Bolas Villain") and the pin never draws them — and no `donor:` line frees them, not even `donor: all`: table-mates are never Donor Decks. A want the sitting cannot cover is reported out loud and resolved by the human — a re-brief (`/tutor:brief`) and a re-run of the affected Seats in seat order — never by taking the copy anyway and never by unbuilding an earlier Seat.

After the last Seat ships, hand the sitting to `/tutor:review` for the Table Review. Upgrading one Seat later is an ordinary Upgrade carrying the Table Brief and the table-mate Deck Blocks: the Seat's own Brief frees its Deck's rows via `donor:` as usual, the Table Brief names the sitting and its seat order, and every other Seat's finished Deck Block rides along as `--table-mate` — the standing table keeps its copies.
